...
const state = {
  ...
  smStream: null, faceLibLoading: null,
};
    ...
    try {
      if (pending2fa) {
        ...
        if (data.need2fa) {
          ...
          $('#loginStep1').hidden = true; $('#demoBox').hidden = true; $('#loginStep2').hidden = false;
...
function navLabel(id) { const k = { overview:'overview',students:'students',teachers:'teachers',timetable:'timetable',reports:'reports',requests:'requests',announce:'announce',audit:'audit',mark:'mark',marks:'marks',sessions:'sessions',attendance:'attendance',selfmark:'selfmark',leave:'leave',child:'child' }[id]; return t(k); }
...
const ST = { P:'present', L:'late', A:'absent' };
...
admin: {
  ...
  timetable: {
    ...
    init: async () => {
      ...
      vp.set('B.Tech', 'CSE', 3);
      $('#vtProgram').addEventListener('change', () => loadAdminTimetable());
      $('#vtBranch').addEventListener('change', () => loadAdminTimetable());
      $('#vtSem').addEventListener('change', () => loadAdminTimetable());
      $('#vtSec').addEventListener('change', () => loadAdminTimetable());
  ...
  reports: {
    ...
    init: () => {
      const rp = wireProgramPicker($('#rgProgram'), $('#rgBranch'), $('#rgSem'));
    ...
    html: () => `
      ...
      <div class="card" id="markCard" hidden>
        ...
        <div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
          ...
          <span class="muted small">Code har 30 sec badalta hai — screenshot share karke dhokha nahi hoga 😄</span></div>
...
async function loadAdminTimetable() {
  ...
  $('#ttAdminView').querySelectorAll('[data-ttsub]').forEach((b) => b.addEventListener('click', safe(async () => {
    const { users } = await api('/api/users?role=teacher');
    const name = prompt('Substitute teacher (name from list):\n' + users.map((u) => u.name).join(', '));
    if (!name) return;
...
async function loadRoster() {
  ...
  renderRoster();
  if (locked) toast('🔒 24h+ purana session — save karne par lock error aayega (admin hi edit kar sakta hai).', 'info');
...
async function captureDescriptor(statusEl) {
  if (!$('#smVideo').srcObject && !state.smStream) {
...
async function registerFace() {
  ...
  try {
    ...
    st.textContent = e.message.includes('face-api') || e.message.includes('Failed') ? '⚠️ Face library load nahi hui (internet?) — selfie-only mode.' : e.message;
...
async function doSelfMark() {
  ...
  let selfie, face;
  try {
    if (state.user.hasFace) {
      face = await captureDescriptor(statusEl);
      selfie = captureSelfie();
    } else if ($('#smSelfie').checked && state.smStream) {
      selfie = captureSelfie();
    }
...
async function makeCertificate() {
  ...
  $('#certHost').innerHTML = `
    <div class="cert-card" id="theCert">
      ...
      <p style="font-size:2rem;font-weight:700;color:#1d4ed8;margin:8px 0;">${esc(String(await api('/api/me/attendance').then((s) => s.percentage)))}%</p>
