/**
 * GL Bajaj Attendance System — v8.3 (Render + Supabase + Excel Import + Custom Sections)
...
async function saveSections(list) {
  const clean = [...new Set(list.map((s) => String(s).trim().toUpperCase()).filter(Boolean))].slice(0, 20);
  if (!clean.length) throw new Error('Kam se kam ek section chahiye.');
...
app.post('/api/users/bulk', requireAuth, requireRole('admin'), H(async (req, res) => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!name && !un && !pw && !branch && !sem) { skippedEmpty++; continue; }
    if (!name || !un || !pw || !program) { errors.push(`Row ${row}: invalid fields (name/username/password/program khaali ya galat)`); continue; }
    if (!PROGRAMS[program].branches.includes(branch)) { errors.push(`Row ${row}: branch "${branch}" ${program} ke liye invalid`); continue; }
    if (!sem || sem < 1 || sem > PROGRAMS[program].sems) { errors.push(`Row ${row}: semester 1–${PROGRAMS[program].sems} hona chahiye`); continue; }
  ...
  res.json({ ok: true, added, skipped, skippedEmpty, errors: errors.slice(0, 15), message: `${added} added · ${skipped} duplicate skip · ${skippedEmpty} empty rows ignored · ${errors.length} invalid.` });
}));
...
(async () => {
  ...
  function startServer(port, tries = 10) {
    const server = app.listen(port, () => {
      console.log('──────────────────────────────────────────────');
      console.log('  🎓 GLBITM Attendance v8.3 — Supabase + Excel Import + Sections');
